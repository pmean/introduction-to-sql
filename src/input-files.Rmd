...
```{r cigarettes}
fn <- "../data/cigarettes.csv"
cig <- read.csv(file=fn, header=TRUE, as.is=TRUE)
head(cig)
...
dbGetQuery(db, "select * from cigarettes where brand like '%&%'")
dbGetQuery(db, "select * from cigarettes where brand not like '% %'")
dbDisconnect(conn=db)
```
...
```{r crawling}
fn <- "../data/crawling.csv"
crawling_data <- read.csv(file=fn, header=TRUE, as.is=TRUE)
head(crawling_data)
db <- dbConnect(SQLite(), dbname="../data/crawling_db.sqlite")
dbWriteTable(conn=db, name="crawling_table", value=crawling_data,
  overwrite=TRUE, row.names=FALSE, header=TRUE)
dbDisconnect(conn=db)

db <- dbConnect(SQLite(), dbname="../data/crawling_db.sqlite")
dbReadTable(db, "crawling_table")
dbDisconnect(conn=db)
```
