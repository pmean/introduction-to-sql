...
In the previous lecture, I showed that there are two ways to account for mismatches in a join. The inner join discards any records where you cannot find a match between the two tables. The left, right, and outer join will include mismatches and add null values as needed.

You should always take the time to get counts before and after a join to help assure that the work is being done correctly. But the default in SQL is to calculate summary statistics before joining. You can fix this using nested queries.

A nested query sounds tricky, but it is just the use of parentheses to control to order of operations. You'll see a few basic examples here, but there are lots of other applications of nested queries in quite different areas.
